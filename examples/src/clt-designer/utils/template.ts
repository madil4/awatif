...
export const template = ({ glulam, nodes, moments, results, analyzeOutputs }) => {
  ...
  const maxEtas = results.val.map(r => Math.max(...r.eta));
  ...
  return html`
    ...
    <table>
      ...
      <tr>
        <td>Slab Height</td>
        <td>${results.val[index].slabHeight}</td>
        <td>mm</td>
      </tr>
      <tr>
        <td>Moment of Inertia</td>
        <td>${results.val[index].inertia.toFixed(0)}</td>
        <td>mm⁴</td>
      </tr>
      <tr>
        <td>Bending Stress</td>
        <td>${results.val[index].bendingStressMax.toFixed(1)}</td>
        <td>N/mm²</td>
      </tr>
      <tr>
        <td>Bending Resistance</td>
        <td>${results.val[index].f_md.toFixed(1)}</td>
        <td>N/mm²</td>
      </tr>
      <tr>
        <td>Maximum Utilization Ratio</td>
        <td>${(results.val[index].etaMax * 100).toFixed(0)}</td>
        <td>%</td>
      </tr>
    </table>
