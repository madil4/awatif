...
describe("CLT fresh-start timing (~1000 DOF)", () => {
  test("reports setup + first solve + total runtime from a fresh model init", () => {
    const stats = measureFreshStartRun();

    expect(stats.dof).toBe(1008);
    expect(stats.setupMs).toBeGreaterThan(0);
    expect(stats.firstSolveMs).toBeGreaterThan(0);
    expect(stats.totalMs).toBeGreaterThan(0);

    // eslint-disable-next-line no-console
    console.log(
      [
        `CLT fresh-start benchmark`,
        `nodes=${stats.nodes}`,
        `elements=${stats.elements}`,
        `dof=${stats.dof}`,
        `freeDof=${stats.freeDof}`,
        `setup=${stats.setupMs.toFixed(2)} ms`,
        `firstSolve=${stats.firstSolveMs.toFixed(2)} ms`,
...
function measureFreshStartRun(): FreshStartStats {
  ...
  const solver = createCachedDeformSolver(nodes, elements, supports, elementInputs);
  const setupMs = performance.now() - setupStart;

  const solveStart = performance.now();
  const deformOutputs = solver.solve(loads, { includeReactions: false });
