...
// --- Main getLocalStiffnessMatrix function --- //
...
// Helper function (can be moved to a common utils header/cpp if needed)
...
// --- Implementation of getLocalStiffnessMatrixFrame --- //
Eigen::MatrixXd getLocalStiffnessMatrixFrame(
    const std::vector<Node> &nodes,
    const ElementInputs &elementInputs,
    int index)
{
    // ... (Frame implementation remains the same) ...
    ...
    double EA_L = (A > 0 && L > 0) ? (E * A) / L : 0;
    double EIz_L3 = (Iz > 0 && L3 > 0) ? (E * Iz) / L3 : 0;
    double EIy_L3 = (Iy > 0 && L3 > 0) ? (E * Iy) / L3 : 0;
    double GJ_L = (J > 0 && L > 0) ? (G * J) / L : 0;
    double EIz_L2 = (Iz > 0 && L2 > 0) ? (E * Iz) / L2 : 0;
    double EIy_L2 = (Iy > 0 && L2 > 0) ? (E * Iy) / L2 : 0;
    double EIz_L = (Iz > 0 && L > 0) ? (E * Iz) / L : 0;
    double EIy_L = (Iy > 0 && L > 0) ? (E * Iy) / L : 0;

    Eigen::MatrixXd kLocal = Eigen::MatrixXd::Zero(12, 12);

    kLocal(0, 0) = EA_L;
    kLocal(0, 6) = -EA_L;

    kLocal(1, 1) = 12 * EIz_L3;
    kLocal(1, 5) = 6 * EIz_L2;
    kLocal(1, 7) = -12 * EIz_L3;
    kLocal(1, 11) = 6 * EIz_L2;

    kLocal(2, 2) = 12 * EIy_L3;
    kLocal(2, 4) = -6 * EIy_L2;
    kLocal(2, 8) = -12 * EIy_L3;
    kLocal(2, 10) = -6 * EIy_L2;
    ...
    kLocal.block<6, 6>(6, 0) = kLocal.block<6, 6>(0, 6).transpose();
    kLocal.block<6, 6>(0, 6) = kLocal.block<6, 6>(6, 0).transpose();
