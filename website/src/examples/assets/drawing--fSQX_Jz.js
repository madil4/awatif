...
const m = o.state([]), v = o.state([]), d = o.state([[5, 5, 0], [10, 15, 0], [15, 10, 0]]), w = o.state([[10, 2, 5], [2, 2, 5], [2, 10, 5], [7, 10, 5]]), h = o.state([]), g = o.state([[0, 1, 2, 3], []]), i = o.state([]), p = o.state([]), y = { width: { value: o.state(2), min: 0.5, max: 5, step: 0.1 } }, E = o.state({ position: [10, 10, 0], rotation: [Math.PI / 2, 0, 0] }), b = 5;
i.val = d.val;
let f = "1st-floor";
function F(t) {
  f = t, E.val = { position: [10, 10, t === "1st-floor" ? 0 : b], rotation: [Math.PI / 2, 0, 0] }, i.val = t === "1st-floor" ? d.val : w.val, p.val = t === "1st-floor" ? h.val : g.val;
}
o.derive(() => {
  f == "1st-floor" && (d.val = i.val, h.val = p.val), f == "2nd-floor" && (w.val = i.val, g.val = p.val);
});
o.derive(() => {
  m.val = [], v.val = [];
  const t = [], n = [];
