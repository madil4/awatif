...
function onParameterChange(parameters2) {
  const radius = parameters2.radius.value;
  const points = parameters2.points.value;
  const circumferences = parameters2.circumferences.value;
  let nodes = [];
  const elements = [];
  for (let circum = 0; circum < circumferences; circum++) {
    const cirum1Angle = 1 / 2 * Math.PI - Math.PI * circum / circumferences;
    const cirum2Angle = 1 / 2 * Math.PI - Math.PI * (circum + 1) / circumferences;
  ...
  nodes = nodes.map((v) => [10 + v[0], 10 + v[1], v[2]]);
