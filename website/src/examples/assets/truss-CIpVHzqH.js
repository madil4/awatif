...
a.derive(() => {
  ...
  for (let t = 0; t <= e; t++) l.push([i * t, 0, w]);
  for (let t = 0; t < e; t++) s.push([t, t + 1]);
  for (let t = 0; t < e; t++) s.push([e + 1 + t, e + 1 + t + 1]);
  for (let t = 0; t <= e; t++) s.push([t, e + 1 + t]);
  for (let t = 0; t < e; t++) t < e / 2 ? s.push([t, e + 1 + t + 1]) : s.push([e + 1 + t, t + 1]);
  const r = { supports: /* @__PURE__ */ new Map([[0, [true, true, true, true, true, true]], [e, [true, true, true, true, true, true]]]), loads: new Map(p.map((t, o) => [o, [0, 0, -O, 0, 0, 0]])) }, u = { elasticities: new Map(s.map((t, o) => [o, y])), areas: new Map(s.map((t, o) => [o, x])) }, m = _(l, s, r, u), S = M(l, s, u, m);
  c.val = l, d.val = s, v.val = r, h.val = u, f.val = m, b.val = S;
});
document.body.append(k(n), I({ mesh: { nodes: c, elements: d, nodeInputs: v, elementInputs: h, deformOutputs: f, analyzeOutputs: b }, settingsObj: { deformedShape: true } }), z({ sourceCode: "https://github.com/madil4/awatif/blob/main/examples/src/truess/main.ts", author: "https://www.linkedin.com/in/madil4/" }));
