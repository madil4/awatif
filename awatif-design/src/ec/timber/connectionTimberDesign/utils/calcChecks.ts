import {
  getTimberProperties,
  getTensileStrength,
} from "./getMaterialProperties"; // Importing functions for material properties (assuming they are defined elsewhere)

// Function to check shear member utilization
export function tensionMemberCheck(
  thickness: number,
  height: number,
  noPerpendicular: number,
  diameter: number,
  tensionForce: number,
  timberGrade: string,
  noSheet: number,
  chi: number,
  thicknessSheet: number
  ...
  // Calculate stress
  const sigma_t0d: number =
    Math.round((tensionForce / A_net) * 1000 * 100) / 100;
...
// Function to check shear member utilization
export function compressionMemberCheck(
  thickness: number,
  height: number,
  noPerpendicular: number,
  diameter: number,
  ...
  const sigma_c0d: number =
    Math.round((compressionForce / A_net) * 1000 * 100) / 100;
...
export function blockFailureCheckAxial(
  steelGrade: string,
  noPerp: number,
  noAxial: number,
  diameter: number,
  ...
  // Calculate design shear resistance
  const V_eff1Rd: number =
    Math.round(
      (((f_u * Ant) / 1.25 + (f_y * Anv) / (Math.sqrt(3) * 1)) / 1000) * 100
    ) / 100;
...
export function getEtaMax(ClassList: any[]): number[] {
  ...
  const fastenerCheckMaxIndex: number =
    fastenerCheckList.indexOf(fastenerCheckMax);
  const tensionMemberCheckMaxIndex: number = tensionMemberCheckList.indexOf(
    tensionMemberCheckMax
  );
  const compressionMemberMaxIndex: number =
    compressionMemberList.indexOf(compressionMemberMax);
  const axialBlockFailureMaxIndex: number =
    axialBlockFailureList.indexOf(axialBlockFailureMax);

  return [
    fastenerCheckMax,
    tensionMemberCheckMax,
    compressionMemberMax,
    axialBlockFailureMax,
    fastenerCheckMaxIndex,
    tensionMemberCheckMaxIndex,
    compressionMemberMaxIndex,
    axialBlockFailureMaxIndex,
  ];
}

export function calcAxialMemberCheck(
  width: number,
  height: number,
  noPerp: number,
  ...
  if (axialForce > 0) {
    ...
    [A_net, f_ct0k, f_ctd, Force, sigma_ct0d, befct, eta] =
      compressionMemberCheck(
        width,
        height,
        noPerp,
        fastenerDiameter,
