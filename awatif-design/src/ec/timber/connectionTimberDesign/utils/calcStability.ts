...
export function calcStability(
    ...
    for (let n = 0; n < L_Md.length; n++) {
        ...
        if (k_crit === 1 && L_lamb_rel[n] < 0.3) {
            eta = (axialForce / f_c0d) ** 2 + L_km[n][0] * L_sigma_md[0] / f_myd + L_km[n][1] * L_sigma_md[1] / f_mzd;
            nw = 'Sp';
        } else if (k_crit === 1 && L_lamb_rel[n] > 0.3) {
            eta = (axialForce) / (f_c0d * L_kc[n]) + L_km[n][0] * L_sigma_md[0] / f_myd + L_km[n][1] * L_sigma_md[1] / f_mzd;
            nw = 'Kn';
        } else if (k_crit < 1 && L_lamb_rel[n] < 0.3) {
            eta = (axialForce) / (f_c0d * L_kc[n]) + (L_sigma_md[0] / (f_myd * L_k_crit[0])) ** L_pot[n][0] + (L_sigma_md[1] / (f_mzd * L_k_crit[1])) ** L_pot[n][1];
            nw = 'Kn/Ki';
        } else {
            eta = 0;
            nw = 'N/A';
