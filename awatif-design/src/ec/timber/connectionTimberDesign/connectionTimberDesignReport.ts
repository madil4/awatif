...
export function connectionTimberDesignReport(
  ...
  let reportSummary = html` <h2>Summary</h2>
  ...
  let reportLoadProperties = html` <h3>Load Properties</h3>
    <p>
      Service class ${renderMath(`${i.serviceClass}`)} and Load duration:
      ${renderMath(`${i.loadDurationClass}`)} result in a modification factor of
      ${renderMath(`k_{mod} = ${o[index].kMod}`)}
    </p>
  ...
  let reportFastenerProperties = html` <h3>Fastener Properties</h3>
    <p class="caption">EN 1995-1-1 Ch. 8.5.1.1</p>
    <p>Characteristic value for the yield moment</p>
    <p>
      ${renderMath(` M_{yRk} = f_{ub} \\cdot d^{2.6} = ${o[index].Myrk} Nmm`)}
    </p>
    <p>Characteristic tensile strength parallel to the grain</p>
    <p>
      ${renderMath(
        ` f_{h0k} = 0.082 \\cdot (1 - 0.01 \\cdot d) \\cdot \\rho_{k} = ${o[index].fh0k} \\frac{N}{mm^2}`
      )}
    </p>
    <br />`;

  let reportSpacings = html` <h3>Spacings</h3>
  ...
  let reportFastenerCapacity = html` <h3>Fastener Capacity</h3>
    <p class="caption">EN 1995-1-1 Ch. 8.2.3-8.11</p>
    <p>Characteristic fastener capacities</p>
    <p class="caption">Equation f</p>
    <p>
      ${renderMath(` F_{vRkf} = f_{hk} \\cdot t_{1}*d = ${o[index].Fvrk_f} kN`)}
    </p>
    <p class="caption">Equation g</p>
    <p>
      ${renderMath(
        ` F_{vRkg} = f_{hk} \\cdot t_{2}*d \\cdot ( \\sqrt{2+ \\frac{4 \\cdot M_{yRk}}{f_{hk} \\cdot d \\cdot t_{1}^2}} ) = ${o[index].Fvrk_g} kN`
      )}
    </p>
    <p class="caption">Equation h</p>
    <p>
      ${renderMath(
        ` F_{vRkh} = 2.3 \\cdot \\sqrt{2 \\cdot M_{yRk} \\cdot f_{hk} \\cdot d} = ${o[index].Fvrk_h} kN`
      )}
    </p>
    <p>Characteristic fastener capacity</p>
    <p>
      ${renderMath(
        ` F_{vRk} = ${Math.max(o[index].Fvrk_1, o[index].Fvrk_2)} kN`
      )}
    </p>
  ...
  let reportFastenerCapacityCheck = html` <h3>Fastener Check</h3>
    <table>
      <tr>
        <th>diameter</th>
  ...
  let reportCompressionCheck = html` <h3>Compression Check</h3>
    ...
    <p>
      ${renderMath(
        "A_{net} = H \\cdot b_{ef} = " + o[index].Anet + " \\text{ cm}^2"
      )}
    </p>
    ...
    <p>
      ${renderMath(
        "\\eta = \\frac{\\sigma_{c0d}}{f_{c0d}} = " +
          (o[index].etaAxialCheck * 100).toFixed(0) +
          " \\%"
      )}
    </p>
    <br />`;

  let reportTensionCheck = html` <h3>Tension Check</h3>
    <p class="caption">EN 1995-1-1 Abs. 6.1.7</p>
  ...
  let reportAxialBlockFailureCheck = html` <h3>Axial Block Failure Check</h3>
    ...
    <p>Effective areas</p>
    <p>
      ${renderMath(` L_{h} = a_{2} \\cdot (n_{perp} - 1) = ${o[index].Lh} `)} mm
    </p>
    <p>
      ${renderMath(
        ` L_{v} = a_{1} \\cdot (n_{axial} - 1) + e_{1} = ${o[index].Lv} `
      )}
      mm
    </p>
    <p>
      ${renderMath(
        ` A_{nt} = (L_{h} - (n_{axial} - 1) \\cdot d_{0}) \\cdot t = ${o[index].Ant} cm^2`
      )}
    </p>
    <p>
      ${renderMath(
        ` A_{nv} = 2 \\cdot (L_{v} - (n_{perp} - 0.5) \\cdot d_{0}) \\cdot t = ${o[index].Anv} cm^2`
      )}
    </p>
    <p>Resistance</p>
    <p>
      ${renderMath(
        ` V_{eff.Rd} = \\frac{f_{u} \\cdot A_{nt}}{1.25} + \\frac{f_{y} \\cdot A_{nv}}{ \\sqrt{3} + 1 } = ${o[index].VeffRd} cm^2`
      )}
    </p>
  ...
  let reportContent = html`
    ${reportHeader} ${reportHeading} ${reportSummary}
    <h2>Input Parameters</h2>
    ${reportLoadProperties} ${reportFastenerProperties} ${reportSpacings}
    ${reportFastenerCapacity}
    <h2>Connection Design</h2>
    ${reportFastenerCapacityCheck} ${reportAxialBlockFailureCheck}
    <h2>Member Design</h2>
    ${reportMemberCheck}
  `;
