...
export function connectionTimberDesignReport(
  ...
  let reportLoadProperties = html` <h3>Load Properties</h3>
    <p>
      ...
      ${renderMath(`k_{mod} = ${o[index][0].kMod}`)}
    </p>
    <p>${renderMath(` \\gamma = 1.3`)}</p>
    <p>Resistance reduction factor</p>
    <p>
      ${renderMath(` \\chi = \\frac{k_{mod}}{\\gamma} = ${o[index][0].chi}`)}
    </p>
    <br />`;
  ...
  let reportFastenerProperties = html` <h3>Fastener Properties</h3>
    ...
    <p>
      ${renderMath(
        ` M_{yRk} = f_{ub} \\cdot d^{2.6} = ${o[index][0].Myrk} Nmm`
      )}
    </p>
    <p>Characteristic tensile strength parallel to the grain</p>
    <p>
      ${renderMath(
        ` f_{h0k} = 0.082 \\cdot (1 - 0.01 \\cdot d) \\cdot \\rho_{k} = ${o[index][0].fh0k} \\frac{N}{mm^2}`
      )}
    </p>
    <br />`;
  ...
  let reportFastenerCapacity = html` <h3>Fastener Capacity</h3>
    ...
    <p>
      ${renderMath(
        ` F_{vRkf} = f_{hk} \\cdot t_{1}*d = ${o[index][0].Fvrk_f} kN`
      )}
    </p>
    <p class="caption">Equation g</p>
    <p>
      ${renderMath(
        ` F_{vRkg} = f_{hk} \\cdot t_{2}*d \\cdot ( \\sqrt{2+ \\frac{4 \\cdot M_{yRk}}{f_{hk} \\cdot d \\cdot t_{1}^2}} ) = ${o[index][0].Fvrk_g} kN`
      )}
    </p>
    <p class="caption">Equation h</p>
    <p>
      ${renderMath(
        ` F_{vRkh} = 2.3 \\cdot \\sqrt{2 \\cdot M_{yRk} \\cdot f_{hk} \\cdot d} = ${o[index][0].Fvrk_h} kN`
      )}
    </p>
    <p>Characteristic fastener capacity</p>
    <p>
      ${renderMath(
        ` F_{vRk} = ${Math.max(o[index][0].Fvrk_1, o[index][0].Fvrk_2)} kN`
      )}
    </p>
  ...
  let reportCompressionCheck = html` <h3>Compression Check</h3>
    ...
    <p>
      ${renderMath(
        "A_{net} = H \\cdot b_{ef} = " + o[index][0].Anet + " \\text{ cm}^2"
      )}
    </p>
    ...
    <p>
      ${renderMath(
        "\\eta = \\frac{\\sigma_{c0d}}{f_{c0d}} = " +
          (o[index][0].etaAxialCheck * 100).toFixed(0) +
          " \\%"
      )}
    </p>
    <br />`;
  ...
  let reportAxialBlockFailureCheck = html` <h3>Axial Block Failure Check</h3>
    ...
    <p>Effective areas</p>
    <p>
      ${renderMath(` L_{h} = a_{2} \\cdot (n_{perp} - 1) = ${o[index][0].Lh} `)}
      mm
    </p>
    <p>
      ${renderMath(
        ` L_{v} = a_{1} \\cdot (n_{axial} - 1) + e_{1} = ${o[index][0].Lv} `
      )}
      mm
    </p>
    <p>
      ${renderMath(
        ` A_{nt} = (L_{h} - (n_{axial} - 1) \\cdot d_{0}) \\cdot t = ${o[index][0].Ant} cm^2`
      )}
    </p>
    <p>
      ${renderMath(
        ` A_{nv} = 2 \\cdot (L_{v} - (n_{perp} - 0.5) \\cdot d_{0}) \\cdot t = ${o[index][0].Anv} cm^2`
      )}
    </p>
    <p>Resistance</p>
    <p>
      ${renderMath(
        ` V_{eff.Rd} = \\frac{f_{u} \\cdot A_{nt}}{1.25} + \\frac{f_{y} \\cdot A_{nv}}{ \\sqrt{3} + 1 } = ${o[index][0].VeffRd} cm^2`
      )}
    </p>
