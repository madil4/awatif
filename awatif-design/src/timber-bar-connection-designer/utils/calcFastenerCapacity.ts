import { getTensileStrength, getTimberProperties } from './getMaterialProperties';

export function characteristicValues(diameter: number, timberGrade: string, strengthClass: string, angle: number): [number, number, number, number] {
    ...
    const f_halphak: number = f_h0k / (k_90 * Math.pow(Math.sin(angle), 2) + Math.pow(Math.cos(angle), 2));
...
export function shearCapacity(diameter: number, thickness: number, timberGrade: string, strengthClass: string, angle: number, chi: number): [number, number, number, number, number, number, number, number, number, number] {
    ...
    // 1 sheet
    const F_vrkf: number = Math.round(f_h0k * thickness * diameter / 1000 * 10) / 10;
    const F_vrkg: number = Math.round(f_h0k * thickness * diameter * (Math.sqrt(2 + (4 * M_yrk) / (f_h0k * diameter * Math.pow(thickness, 2))) - 1) / 1000 * 10) / 10;
    const F_vrkh: number = Math.round(2.3 * Math.sqrt(M_yrk * f_h0k * diameter) / 1000 * 10) / 10;

    // 2 sheets
    const F_vrkl: number = Math.round(f_h0k * thickness * diameter / 1000 * 10) / 10;
